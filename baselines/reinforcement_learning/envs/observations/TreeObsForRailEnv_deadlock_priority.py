...
class TreeObsForRailEnv(ObservationBuilder):
    ...
    def get(self, handle: int = 0) -> Node:
        ...
        #13:
            delay between projected arrival time and latest arrival time
        #14:
            priority of the agent based on some heuristic
        ...
        root_node_observation = Node(dist_own_target_encountered=0, dist_other_target_encountered=0,
                                                       ...
                                                       time_delay_to_target=agent.get_current_delay(self.env._elapsed_steps, self.env.distance_map),
                                                       priority=0,
                                                       childs={})
    ...
    def _explore_branch(self, handle, position, direction, tot_dist, depth):
        ...
        node = Node(dist_own_target_encountered=own_target_encountered,
                                      ...
                                      time_delay_to_target=agent.get_current_delay(self.env._elapsed_steps, self.env.distance_map),
                                      priority=self.priorities[handle],
                                      childs={})
    ...
    """
    def _build_shortest_path(self, handle):
        agent = self.env.agents[handle]
        pos = agent.initial_position
        dir = agent.initial_direction

        dist_min_to_target = self.env.obs_builder.rail_graph.dist_to_target(handle, pos[0], pos[1], dir)
        ...
        while dist_min_to_target:
            ...
            for new_dir in range(4):
                if possible_transitions[new_dir]:
                    ...
                    new_min_dist = self.env.obs_builder.rail_graph.dist_to_target(handle, new_pos[0], new_pos[1], new_dir)
    ...
    def blocked_agents(self):
        blocked_path = {}
        shortest_paths = get_shortest_paths(self.env.distance_map) # List[WalkingElement]
        agent_list = []
        speed_list = []
        for agent in self.env.agents:
            blocked_path.setdefault(agent.handle, set([]))
            if agent.state in [TrainState.MALFUNCTION_OFF_MAP, TrainState.WAITING, TrainState.DONE]:
                continue
            agent_list.append(agent.handle)
            speed_list.append(agent.speed_counter.speed)

        visited = {}
        for (agent, speed) in zip(agent_list, speed_list):
            shortest_path = shortest_paths[agent]
            if shortest_path is None:
                continue
            #print("shortest path", shortest_path)
            time = 0
            for step in shortest_path:
                if step.position in visited.keys():
                    for potential_conflict in visited[step.position]:
                        if np.abs(potential_conflict["time"]-i)<=2 and potential_conflict["direction"]!=step.direction:
                            # an agent is at the same position more or less at the same time as our agent in an opposite direction ==> deadlock
                            conflicting_agent = potential_conflict["agent"]
                            blocked_path[agent].add(conflicting_agent)
                            blocked_path[conflicting_agent].add(agent)   
                    for i in range(int(1/speed)):    
                        visited[step.position].append({"time":time, "agent": agent, "direction": step.direction})
                        time += 1
                else:
                    for i in range(int(1/speed)):    
                        visited[step.position] = [{"time":time, "agent": agent, "direction": step.direction}]
                        time += 1
    ...
    def calculate_priorities(self):
        self.priorities = {}
        active_agents = [a.handle for a in self.env.agents if a.state == TrainState.MOVING]

        blocked_path = self.blocked_agents()
        #print("blocked_path", blocked_path)
        for a in self.env.agents:
            if a.handle in active_agents:
                try:
                    self.priorities[a.handle] = len(blocked_path[a.handle])
                except:
                    self.priorities[a.handle] = 0
            else:
                self.priorities[a.handle] = 0 # train is either malfunctionning or not departed yet --> no priority
