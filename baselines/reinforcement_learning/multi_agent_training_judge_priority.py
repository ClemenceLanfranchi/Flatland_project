...
from reinforcement_learning.ppo_agent_judge_priority import FLATLandPPOPolicy
from utils.agent_action_config import get_flatland_full_action_size, get_action_size, map_actions, \
    map_action, \
...
from utils.flatland_observation_judge_priority import FlatlandObservation, FlatlandTreeObservation , \
...
try:
    import wandb
    wandb.init(sync_tensorboard=True, project="flatland3-ppo-final", entity="paris-brest")
except ImportError:
...
def train_agent(train_params, train_env_params, eval_env_params, obs_params):
    ...
    device = torch.device("cuda") if train_params.use_gpu else torch.device('cpu')
    print("device", device)
    ...
    if train_params.use_observation == 'TreeObs':
        ...
        tree_observation = FlatlandTreeObservation(device=device, max_depth=observation_tree_depth)
    else: #other observations are not compatible with the Judge yet
        print("Using FlatlandTreeObservation (standard)")
        tree_observation = FlatlandTreeObservation(device=device, max_depth=observation_tree_depth)
    # Get the state size
    ...
    if train_params.policy == "DDDQN":
        ...
        policy = FLATLandPPOPolicy(state_size, get_action_size(), use_replay_buffer=False,
                                   in_parameters=train_params)
    ...
    for episode_idx in range(n_episodes + 1):
        ...
        # See details in flatland.envs.line_generators.line_generator
        ...
        for step in range(max_steps - 1):
            ...
            for agent_handle in train_env.get_agent_handles():
                dones[agent_handle] = (train_env.agents[agent_handle].state == TrainState.DONE)
            env_step_timer.end()

            # policy.shape_reward -------------------------------------------------------------------------------------
            policy_shape_reward_timer.start()
            # Deadlock
            deadlocked_agents, all_rewards, = find_and_punish_deadlock(train_env, all_rewards,
                                                                       -10.0)
        ...
        policy.end_episode(train=True, env=train_env)
        ...
        try:
            ...
            print("ERROR!!!!!!!!!!!! since Flatland 3")
            print('score', score)
            print('all_rewards', all_rewards)
            print('train_env.get_num_agents()', train_env.get_num_agents())
        ...
        if episode_idx % checkpoint_interval == 0 and episode_idx > 0:
            policy.save(train_params.checkpoints_path + '/checkpoints/' + training_id + '-' + str(episode_idx) + '.pth', train_env)
        ...
        if episode_idx > 49:
            try:
                ...
                writer.add_scalar("training/judge_loss", train_env.obs_builder.tree_obs.judge.loss, episode_idx)
...
def eval_policy(env, tree_observation, policy, train_params, obs_params):
    ...
    for episode_idx in range(n_eval_episodes):
        ...
        policy.end_episode(train=False, env=env)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoints_path", help="path to the checkpoints directory", default="..", type=str)
