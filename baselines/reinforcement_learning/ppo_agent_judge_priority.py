...
class PPOPolicy(LearningPolicy):
    ...
    def train_net(self, env):
        ...
        judge_target, judge_states = env.obs_builder.tree_obs.judge.get_rollout()
        env.obs_builder.tree_obs.judge.optimize((judge_target, judge_states))
    
    def end_episode(self, train, env):
        if train:
            self.train_net(env)
    ...
    def save(self, filename, env):
        ...
        env.obs_builder.tree_obs.judge.save_judge(filename)
...
class FLATLandPPOPolicy(PPOPolicy):
    ...
    def end_episode(self, train, env):
        super(FLATLandPPOPolicy, self).end_episode(train, env)
        self.deadlocked_agent = {}
