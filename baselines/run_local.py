...
from reinforcement_learning.envs.utils.deadlock_check import check_if_all_blocked

base_dir = Path(__file__).resolve().parent.parent
sys.path.append(str(base_dir))

parser = ArgumentParser()
parser.add_argument("--checkpoint", type=str, default="baselines/checkpoints/ppo-default-smallv0-9800.pth")
parser.add_argument("--tree_obs_type", choices=["default", "deadlock", "judge"], default="default")
...
tree_obs_type = args.tree_obs_type
...
if tree_obs_type == "default":
    from flatland.envs.observations import TreeObsForRailEnv
    from reinforcement_learning.ppo_agent import PPOPolicy
    from utils.observation_utils import normalize_observation
    tree_observation = TreeObsForRailEnv(max_depth=observation_tree_depth, predictor=predictor)
elif tree_obs_type == "deadlock":
    from reinforcement_learning.envs.observations.TreeObsForRailEnv_deadlock_priority import TreeObsForRailEnv
    from reinforcement_learning.ppo_agent import PPOPolicy
    from utils.observation_utils_priority import normalize_observation
    tree_observation = TreeObsForRailEnv(max_depth=observation_tree_depth, predictor=predictor)
elif tree_obs_type == "judge":
    from reinforcement_learning.envs.observations.TreeObsForRailEnv_Judge_priority import TreeObsForRailEnv
    from reinforcement_learning.envs.observations.Judge import Judge
    from reinforcement_learning.ppo_agent_judge_priority import PPOPolicy
    from utils.observation_utils_priority import normalize_observation
    tree_observation = TreeObsForRailEnv(max_depth=observation_tree_depth, \
        predictor=predictor, judge = Judge(lr=1e-4, batch_size=8, optimization_epochs=3, device=torch.device('cpu')))
...
policy = PPOPolicy(state_size, action_size)

if os.path.isfile(checkpoint+".value"):
    policy.load(checkpoint)
    # tree_observation.judge.load_judge(checkpoint+".judge")
else:
...
while True:
    ...
    local_env = env #remote_client.env
    ...
    while True:
        try:
            ...
            if not check_if_all_blocked(env=local_env):
                ...
                try:
                    _, all_rewards, done, info = env.step(action_dict) #remote_client.env_step(action_dict)
                except:
                ...
                try:
                    _, all_rewards, done, info = env.step(action_dict) #remote_client.env_step({})
                except:
