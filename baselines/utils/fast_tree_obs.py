...
class FastTreeObs(ObservationBuilder):
    ...
    def calculate_rail_agent_state(self):
    ...
    def project_possible_paths_as_probabilistic(self,
        ...
        if agent.state == agent_state_filter:
            agent_virtual_position, agent_virtual_direction, agent_state, agent_target = \
    ...
    def get_density_of_projected_path_to_target(self):
        ...
        for agent_handle in self.env.get_agent_handles():
            ...
            if agent_state < TrainState.DONE:
                ...
                if fast_count_nonzero(possible_transitions) == 1 and \
                        ...
                        cache.status == agent_state:
                    # use case
                    prob_shortest_path_map = cache.shortest_path_map
                    prob_branching_path_map = cache.branching_path_map
                    for dir_loop in range(4):
                        prob_shortest_path_map[dir_loop, agent_virtual_position[0], agent_virtual_position[1]] = 0
                        prob_branching_path_map[dir_loop, agent_virtual_position[0], agent_virtual_position[1]] = 0
                else:
                    prob_shortest_path_map, prob_branching_path_map = \
                        self.project_possible_paths_as_probabilistic(agent_handle, agent_state, break_max_step)
        ...
        for agent_handle in self.env.get_agent_handles():
            ...
            agent_virtual_position, agent_virtual_direction, agent_state, agent_target = \
                self.agent_can_choose_helper.get_agent_position_and_direction(agent_handle)
            if agent_state in [TrainState.STOPPED, TrainState.MOVING, TrainState.MALFUNCTION]:
            ...
            elif agent_state == TrainState.READY_TO_DEPART:
    ...
    def get_many(self, handles: Optional[List[int]] = None):
        ...
        self.calculate_rail_agent_state()
    ...
    def get(self, handle: int = 0):
        ...
        if agent_state == TrainState.READY_TO_DEPART:
            observation[0] = 1
        elif agent_state in [TrainState.MOVING, TrainState.STOPPED, TrainState.MALFUNCTION]:
            observation[1] = 1
        else:
            observation[2] = 1
            agent_done = True
