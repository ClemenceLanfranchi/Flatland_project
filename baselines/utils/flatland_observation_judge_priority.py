...
class FlatlandTreeObservation(ObservationBuilder):
    def __init__(self, device, max_depth: int):
        super(FlatlandTreeObservation, self).__init__()
        self.max_depth = max_depth
        self.predictor = ShortestPathPredictorForRailEnv(self.max_depth)
        self.tree_obs = TreeObsForRailEnv(max_depth=max_depth, predictor=self.predictor, judge = Judge(lr=1e-4, batch_size=8, optimization_epochs=3, device=device))
